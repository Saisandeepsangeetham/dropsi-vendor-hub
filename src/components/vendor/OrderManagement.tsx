          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="text-center p-4 bg-blue-50 rounded-lg">
              <div className="text-2xl font-bold text-blue-600">
                {product.orders.reduce((acc, order) => acc + order.customerCount, 0)}
              </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">{product.requiredStock} {product.unit}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Remaining</span>
                <span className={cn(
                  "font-medium",
                  product.currentStock - product.requiredStock >= 0 ? "text-green-600" : "text-red-600"
                )}>
...
export const OrderManagement: React.FC = () => {
  ...
  const categories = ['all', ...Array.from(new Set(mockPackagingData.map(item => item.category)))];

  const filteredData = filterCategory === 'all' 
    ? mockPackagingData 
    : mockPackagingData.filter(item => item.category === filterCategory);

  const totalItems = filteredData.reduce((acc, item) => acc + item.totalQuantity, 0);
  const totalCustomers = filteredData.reduce((acc, item) => 
    acc + item.orders.reduce((orderAcc, order) => orderAcc + order.customerCount, 0), 0
  );

  const stockIssues = filteredData.filter(item => item.stockStatus !== 'sufficient').length;
      ...
      <div className="min-h-screen bg-white p-8 print:p-4">
        <div className="max-w-4xl mx-auto">
          <div className="flex justify-between items-center mb-8 print:mb-4">
            <div>
              <h1 className="text-2xl font-bold">Daily Packaging List</h1>
              <p className="text-muted-foreground">Date: {format(selectedDate, 'PPP')}</p>
            </div>
            <Button 
          ...
          <div className="space-y-6">
            {categories.slice(1).map(category => {
              ...
              return (
                <div key={category}>
                  <h2 className="text-lg font-semibold mb-3 border-b pb-2">{category}</h2>
                      ...
                      <div key={item.id} className="flex justify-between items-center py-2 border-b border-dashed">
                        <div>
                          ...
                          <div className="text-sm text-muted-foreground">
                            {item.orders.reduce((acc, order) => acc + order.customerCount, 0)} customers
                          </div>
                        </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Daily packaging forecast and order tracking</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-medium">Packaging Date</CardTitle>
          </CardHeader>
          <CardContent>
            <Popover>
                ...
                <div className="flex-1">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Badge 
                        variant="outline" 
                        className={cn("capitalize", getStockStatusColor(item.stockStatus))}
                      >
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-4">
                    ...
                    <div>
                      <div className="text-sm text-muted-foreground">Customers</div>
                      <div className="font-semibold text-lg">
                        {item.orders.reduce((acc, order) => acc + order.customerCount, 0)}
                      </div>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <div className={cn(
                        ...
                        item.currentStock - item.requiredStock >= 0 ? "text-green-600" : "text-red-600"
                      )}>
      ...
      {filteredData.length === 0 && (
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <h3 className="text-lg font-medium mb-2">No orders for this date</h3>
